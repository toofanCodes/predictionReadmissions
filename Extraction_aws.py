...
import json
import hashlib
from s3_utils import read_csv_from_s3, write_df_to_s3
...
def extraction(path, typeOfOp=0):
    ...
    if not createSchema(path, typeOfOp):
        ...
        if path.startswith("s3://"):
            ...
            _, _, bucket, *key_parts = path.split("/")
            key = "/".join(key_parts)
            rawDf = read_csv_from_s3(bucket, key)
        else:
        ...
        write_df_to_s3(rawDf, bucket, 'processed/feature_admissions.csv')
