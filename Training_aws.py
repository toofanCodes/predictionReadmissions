...
def predict_with_model(table_name, save_to_db=True, save_to_s3=False, s3_bucket=None, s3_key=None):
    engine = connectToDB()
    if engine is None:
        print("Database connection failed.")
        return
    ...
    # Save predictions to a new table in DB
    if save_to_db:
    ...
    if save_to_s3 and s3_bucket and s3_key:
        write_df_to_s3(df[['encounter_id', 'patient_nbr', 'prediction', 'probability']], s3_bucket, s3_key)
        print(f"Predictions saved to s3://{s3_bucket}/{s3_key}")
